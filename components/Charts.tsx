...
// Valkea — bağımlılıksız (saf SVG) grafik bileşenleri. Teal #61aebd / amber #e5ab53.
    ...
    <svg width="100%" viewBox={`0 0 ${W} ${height}`} preserveAspectRatio="none" style={{ display: "block" }}>
      <defs>
        <linearGradient id="netTrendFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#61aebd" stopOpacity="0.35" />
          <stop offset="100%" stopColor="#61aebd" stopOpacity="0" />
        </linearGradient>
      </defs>
      {/* sıfır çizgisi */}
      ...
      <polyline points={linePts} fill="none" stroke="#61aebd" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      {points.map((p, i) => {
        const isLast = i === points.length - 1;
        return (
          <g key={i}>
            <circle cx={x(i)} cy={y(p.net)} r={isLast ? 4 : 3} fill={isLast ? "#e5ab53" : "#61aebd"} stroke="#fff" strokeWidth="1.5" />
